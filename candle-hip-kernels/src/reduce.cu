...
//         max_val = maxg(max_val, __shfl_xor(static_cast<int>(max_val), mask, 32));
    }
...
//         tmp += __shfl_xor(static_cast<int>(tmp), mask, 32);
    }
...
fast_max(const size_t src_numel, const size_t el_to_sum_per_block,
  ...
  shr[tid] = std::numeric_limits<T>::min();
  // Elements summed in this block range from dst_id * el_to_sum_per_block
  // to (dst_id + 1) * el_to_sum_per_block.
  size_t start_idx = dst_id * el_to_sum_per_block;
  size_t stop_idx = min(start_idx + el_to_sum_per_block, src_numel);
  ...
  while (idx < stop_idx) {
    // TODO: Fast version for the contiguous case.
    size_t strided_i = get_strided_index(idx, num_dims, dims, strides);
    shr[tid] = MAX(shr[tid], src[strided_i]);
    idx += blockDim.x;
  }
...
fast_min(const size_t src_numel, const size_t el_to_sum_per_block,
  ...
  shr[tid] = std::numeric_limits<T>::max();
  // Elements summed in this block range from dst_id * el_to_sum_per_block
  // to (dst_id + 1) * el_to_sum_per_block.
  size_t start_idx = dst_id * el_to_sum_per_block;
  size_t stop_idx = min(start_idx + el_to_sum_per_block, src_numel);
